...
export const calculatePercentage = async (
  ...
  try {
    // Validate cohortId before making API call
    if (
      !cohortMemberRequest?.filters?.cohortId ||
      cohortMemberRequest.filters.cohortId === "" ||
      cohortMemberRequest.filters.cohortId === "all"
    ) {
      console.warn(
        "calculatePercentage: Invalid cohortId",
        cohortMemberRequest?.filters?.cohortId
      );
      return {};
    }
    ...
    if (!response || !response.result) {
      console.warn(
        "calculatePercentage: Invalid response from getMyCohortMemberList",
        response
      );
      return {};
    }

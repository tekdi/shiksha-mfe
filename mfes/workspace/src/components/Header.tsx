...
const Header: React.FC<HeaderProps> = ({ toggleDrawer, openDrawer }) => {
  ...
  // Retrieve stored userId and language, and sync with router locale
  useEffect(() => {
    if (typeof window !== "undefined" && window.localStorage) {
      ...
      // Sync with router locale (priority) or stored language
      const currentLocale = router.locale || storedLanguage || "en";
      setSelectedLanguage(currentLocale);
      setLanguage(currentLocale);

      // Update localStorage if router locale is different
      if (router.locale && router.locale !== storedLanguage) {

...
const UserAttendanceHistory = () => {
  ...
  const pathname = router.pathname; // Use router.pathname instead of usePathname() for consistency
    ...
    const initializePage = async () => {
      if (typeof window !== "undefined" && window.localStorage) {
        ...
        if (token && storedUserId) {
          ...
          // Only update userId if it's different (already set from initial state)
          if (userId !== storedUserId) {
            setUserId(storedUserId);
          }

          // Set classId from query params, localStorage, or fetch from API
          if (query.classId) {
            ...
            try {
              ...
              if (response && response.length > 0) {
                ...
                // Extract centers and batches similar to SimpleTeacherDashboard
                const centers = response
                  .filter(
                    (center: any) => center.cohortName && center.childData
                  )
                  .map((center: any) => ({
          ...
          // Only redirect if we're not already on attendance-history page
          // and if the year is inactive
          if (!isActiveYear) {
  ...
  useEffect(() => {
    const getAttendanceStats = async () => {
      if (
        ...
        // Reset attendance stats if classId is invalid
        setPercentageAttendance({});
      }
    };
    getAttendanceStats();

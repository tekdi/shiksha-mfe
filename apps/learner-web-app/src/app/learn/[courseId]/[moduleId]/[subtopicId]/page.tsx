...
// const ArrowForwardIcon = ({ color = PRIMARY }: { color?: string }) => (
...
function LessonNode({ lesson, courseId, moduleId, subtopicId, statusData, isUnlocked, router, t }: any) {
  ...
  const handleClick = () => {
    ...
    if (hasChildren) { setExpanded(v => !v); }
      ...
      if (isCFL) return;
      router.push(`/learn/${courseId}/${moduleId}/${subtopicId}/${lesson.identifier || lesson.id}${query}`); 
    }
  };
  ...
  return (
    <Box sx={{ mb: 1 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
        ...
        >
          <Box sx={{ flex: 1 }}>
            <Typography sx={{ fontWeight: 700, fontSize: 12,fontFamily:'Inter', color: '#1A1A1A', lineHeight: 1.3 }}>{lesson.name}</Typography>
            <Typography sx={{ fontSize: 10,fontWeight:400, color: '#9999999', mt: 0.3, fontFamily:'Inter' }}>
              {hasChildren
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', color: isCompleted ? SUCCESS : PRIMARY }}>
            {isUnlocked && (hasChildren
              ...
              : <ArrowForwardIcon color={isCompleted ? SUCCESS : PRIMARY} />
            )}
          </Box>
        </Box>
      </Box>
...
export default function SubtopicPage() {
  ...
  return (
    <Box sx={{ minHeight: '100dvh', bgcolor: 'background.default', display: 'flex', flexDirection: 'column', width: '100%' }}>
      ...
      {isCFL ? (
        <CFLHeader title={trainerName || "Trainer Progress"} showBack onBack={() => router.back()} />
      ) : (
      ...
      <Box sx={{ px: 2, py: 2, flex: 1, pb: 22 }}>
        ...
        {subtopicComplete && (
          <Box sx={{ bgcolor: '#ECF5EE', borderRadius: '12px', p: 3, textAlign: 'center', mb: 3, border: '1px solid #E0E0E0' }}>
            ...
            <Typography sx={{ fontWeight: 700, fontSize: 22, color: '#2E7D32', mb: 0.5,fontFamily:'Open sans' }}>{t('LEARNER_APP.LEARN.SUBTOPIC_COMPLETE')}</Typography>
          </Box>
        )}
        ...
        {subtopicComplete && !isModuleComplete && nextSubtopic && (
          <Box sx={{ mb: 3 }}>
            <Typography sx={{ fontWeight: 700, fontSize: 14, color: '#1A1A1A', mb: 1,fontFamily:'Open sans', textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('LEARNER_APP.LEARN.UP_NEXT')}</Typography>
            <Box sx={{ borderRadius: '16px', border: `1.5px solid ${Math.round(getNodeCompletionPercent(nextSubtopic, statusData)) >= 70 ? '#388E3C' : PRIMARY}`, bgcolor: '#fff', overflow: 'hidden' }}>
              ...
              {(nextSubtopic.children || []).map((lesson: any, idx: number) => (
                <Box key={lesson.identifier} onClick={() => router.push(`/learn/${courseId}/${moduleId}/${nextSubtopic.identifier}/${lesson.identifier}`)} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 2, py: 1.25, borderBottom: idx < (nextSubtopic.children || []).length - 1 ? '1px solid #F9FAFB' : 'none', cursor: 'pointer', '&:hover': { bgcolor: '#FFF7F0' } }}>
                  ...
                  <ArrowForwardIcon sx={{ color: isNodeDone(lesson, statusData) ? '#388E3C' : PRIMARY }} />
                </Box>
              ))}
            </Box>
          </Box>
        ...
        <Box sx={{ mb: 2, borderRadius: '16px', overflow: 'hidden', border: `1.5px solid ${subtopicComplete ? SUCCESS : '#E5E7EB'}`, bgcolor: 'background.paper' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 2, py: 2 }}>
            <Box sx={{ flex: 1 }}>
              <Typography sx={{ fontWeight: 700, fontSize: 12, color: '#1A1A1A',fontFamily:'Inter' }}>{subtopicName}</Typography>
              <Typography sx={{fontFamily:'Inter', fontSize: 10, color: '#999999', mt: 0.3,fontWeight:400 }}>
                {t('LEARNER_APP.LEARN.COMPLETED_LESSONS', { completed: lessons.filter(l => isNodeDone(l, statusData)).length, total: lessons.length })}
              </Typography>
              {/* {subtopicDescription && <Typography sx={{ fontFamily: 'Inter', fontSize: 10,fontWeight:400, color: '#999999', mt: 0.8 }}>{subtopicDescription}</Typography>} */}
            </Box>
          </Box>
      ...
      {(() => {
        ...
        const showModuleCompletion = subtopicComplete && !nextSubtopic && !nextModule;
        
        if (isCFLUser && !showModuleCompletion) return null;
        ...
        return (
          <Box sx={{ position: 'fixed', bottom: 65, left: 0, right: 0, px: 2, py: 1.5, bgcolor: '#fff', borderTop: '1px solid #F3F4F6', zIndex: 10 }}>
            {showModuleCompletion ? (
              <Button fullWidth variant="contained" onClick={() => router.push(`/learn/${courseId}/${moduleId}?view=module_completion`)} sx={{ bgcolor: PRIMARY, color: '#fff', borderRadius: '12px',fontFamily:'Open Sans', fontWeight: 600, textTransform: 'none', py: 1.5, fontSize: 15 }}>
               {t('COMMON.CONTINUE')}
              </Button>
            ) : subtopicComplete && !nextSubtopic && nextModule ? (
              <Button fullWidth variant="contained" onClick={() => router.push(`/learn/${courseId}/${nextModule.identifier}`)} sx={{ bgcolor: PRIMARY, color: '#fff', borderRadius: '12px',fontFamily:'Open Sans', fontWeight: 600, textTransform: 'none', py: 1.5, fontSize: 15 }}>
              ...
              <Button fullWidth variant="contained" onClick={() => router.push(`/learn/${courseId}/${moduleId}/${nextSubtopic.identifier}`)} sx={{ bgcolor: PRIMARY, color: '#fff', borderRadius: '12px', fontWeight: 800, textTransform: 'none', py: 1.5, fontSize: 15 }}>
                {t('LEARNER_APP.LEARN.START_MODULE', { moduleName: nextSubtopic.name })}
              </Button>
            ) : firstIncomplete ? (
              <Button fullWidth variant="contained" onClick={() => router.push(`/learn/${courseId}/${moduleId}/${subtopicId}/${firstIncomplete.identifier}`)} sx={{ bgcolor: PRIMARY, color: '#fff', borderRadius: '12px', fontWeight: 800, textTransform: 'none', py: 1.5, fontSize: 15 }}>
                {t('LEARNER_APP.LEARN.START_MODULE', { moduleName: firstIncomplete.name })}
              </Button>
            ) : null}
          </Box>
        );
